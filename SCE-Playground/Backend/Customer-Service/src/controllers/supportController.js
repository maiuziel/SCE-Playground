...
export async function markResponseAsRead(req, res) {
  ...
  export async function addClientComment(req, res) {
    ...
    try {
      ...
      res.status(200).json({ message: 'Comment saved', request });
    } catch (err) {
  ...
  export const getNewClientRequests = async (req, res) => {
    try {
      ...
      console.error('Failed to fetch new client requests:', error);
      res.status(500).json({ error: 'Failed to fetch new client requests' });
    }
  };
  
  export const markAllNewRequestsAsRead = async (req, res) => {
    try {
      ...
      console.error('Failed to mark new requests as read:', error);
      res.status(500).json({ error: 'Failed to mark new requests as read' });
    }
  };
